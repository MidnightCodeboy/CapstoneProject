...
/**
 *  Subclass of Scene. Creates an interactive environment that display menu options,
...
public class MenuScene extends Scene{
     ...
     */
    public MenuScene(Dimension size, String backgroundImagePath, Keys keys){
    ...
    public void update() {
        ...
        if (entryDelay == 0) {
            if (keys.isKeyPressed(KeyEvent.VK_I)) {
                setNextScene("game");
            } else if (keys.isKeyPressed(KeyEvent.VK_P)){
                setNextScene("credits");
            } else if (keys.isKeyPressed(KeyEvent.VK_O)){
                setNextScene("controls");
            }
        }
    }
